...
// Middleware to attach io to req (optional if using global io)
// router.use((req, res, next) => {
//     req.io = req.app.get('io'); // Attach io to req
//     next();
// });
...
router.post('/register',   
 async (req, res) => {
    ...
    const feesPaidBoolean = feesPaid === 'on';
    ...
    try {
        ...
        if (req.io) {
            req.io.emit('memberAdded', newMember);
        } else {
            console.error('req.io not defined');
        }
...
router.post('/mark-completed', async (req, res) => {
  ...
  try {
    ...
    // Emit event
    req.io.emit('memberCompleted', { memberId, month, year });
...
router.post('/mark-paid', async (req, res) => {
  ...
  try {
    await Member.findById(memberId, async (err, member) => {
      ...
      await member.save();
      req.io.emit('memberCompleted', { memberId, month, year }); // Emit event
      res.redirect('/admin/dashboard');
    });
  } catch (error) {
    console.error(error);
